...
kubeconfig 配置文件 （ 类似于 k8s 集群的管理员，能看懂配置信息就行！ ）
...
- RoleBinding (空间绑定)
  ...
  - 只能管理**受限于命名空间的资源**（比如 Pod, Deployment, Service）

- ClusterRoleBinding (集群绑定)

  - 不仅能管理名称空间的资源，而且能管理集群级别的资源 （比如 **Nodes (物理节点)**、**PersistentVolumes (存储卷)**、**Namespaces 本身**）

- 混合绑定

  - ClusterRole + RoleBinding  把全局定义的模板，应用在局部空间里；

  - 写一个 `ClusterRole` 叫 `common-read-only`。

    在 50 个 Namespace 里创建 `RoleBinding` 引用它。

    以后想加权限？改一下那个 `ClusterRole`，50 个 Namespace 瞬间同步生效！
...
以上创建的用户账号都是 UA ，不管是 cert 文件还是 token 都不是 k8s 的资源，都是在 k8s 之外创建的，是给人用的！

 每一个名称空间都有一个 default 的 SA 账号；

在 deploy 的 yaml 文件中 spec.template.spec 字段下可以看到 serviceAccountName: default ，这个就是默认 SA 账号，这个 SA 账号只有一般权限；

创建 SA 账号，也是在 spec.template.spec 这个字段下操作；
...
# 命令创建 role ：创建一个对所有pod,service,deployment具有get、list权限的role的资源定义文件格式
...
### ✅ 方案：使用 `ClusterRole` + `RoleBinding`（推荐生产方式）
